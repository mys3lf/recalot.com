...
package com.recalot.views.onlineexperiments;
        ...
        try {
            if (pathInfo != null && !pathInfo.equals("") && !pathInfo.equals("/")) {
                ...
                switch (split.length) {
                    case 1: {
                        switch (method) {
                            case GET: {
                                ...
                                result = this.handler.process(ExperimentsController.ExperimentsRequestAction.GetOnlineExperiment, templateKey, params);
                                break;
                            }
                            case DELETE: {
                                params.put(Helper.Keys.ExperimentId, split[0]);
                                result = this.handler.process(ExperimentsController.ExperimentsRequestAction.DeleteOnlineExperiment, templateKey, params);
                                break;
                            }
                        }
                    }
                    ...
                    case 2: {
                        String id = split[0];
                        if (split[1].toLowerCase().equals("rec") && method == HTTPMethods.GET) {
                            params.put(Helper.Keys.ExperimentId, id);
                            result = this.handler.process(ExperimentsController.ExperimentsRequestAction.GetRecommendationForOnlineExperiment, templateKey, params);
                        }
                    }
                    break;
                }
                ...
                switch (method) {
                    case GET: {
                        result = handler.process(ExperimentsController.ExperimentsRequestAction.GetOnlineExperiments, templateKey, params);
                        break;
                    }
                    case PUT: {
                        result = this.handler.process(ExperimentsController.ExperimentsRequestAction.CreateOnlineExperiment, templateKey, params);
                        break;
                    }
                }
            }
