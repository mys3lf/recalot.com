 ...
 */
@Component
@Service({DataAccessController.class, Controller.class})
public class DataAccessController implements com.recalot.common.interfaces.controller.DataAccessController, Closeable {
	...
	private GenericServiceListener<DataTemplate> dataTemplates;
	...
	private TemplateResult getDataSourceBuilder(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		...
		if (builder.getConfiguration(Helper.Keys.DataBuilderId) == null) {
			builder.setConfiguration(
					new ConfigurationItem(Helper.Keys.DataBuilderId, ConfigurationItem.ConfigurationItemType.String,
							builder.getKey(), ConfigurationItem.ConfigurationItemRequirementType.Hidden));
		}

		return template.transform(builder);
	}

	private TemplateResult getData(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
	...
	private TemplateResult getRelations(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult createRelation(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult updateRelation(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult getUser(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult getUsers(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult updateUser(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult createUser(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult getItems(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult getItem(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult updateItem(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult deleteItem(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult createItem(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult addInteraction(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
	...
	private TemplateResult getInteractions(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult getInteraction(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource dataSource = getDataSource(access, param.get(Helper.Keys.SourceId));
		if (dataSource.getState() != DataInformation.DataState.READY)
			throw new NotReadyException("The datasource %s is not yet ready.", dataSource.getId());
	...
	private TemplateResult getSources(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
	...
	private TemplateResult createSource(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		DataSource source = access.createDataSource(param);
		return template.transform(source);
	}

	private TemplateResult updateSource(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		String id = param.get(Helper.Keys.SourceId);
	...
	private TemplateResult getSource(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		String id = param.get(Helper.Keys.SourceId);
	...
	private TemplateResult deleteSource(DataAccess access, DataTemplate template, Map<String, String> param)
			throws BaseException {
		String id = param.get(Helper.Keys.SourceId);
