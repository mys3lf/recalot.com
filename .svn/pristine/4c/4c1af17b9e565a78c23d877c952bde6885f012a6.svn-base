var Recalot = {
				...
				for(var i = 0; i < data.length ; i++){
					...
					for(var j = 0; j < items.length; j++){
						if(data[i].itemId == items[j].itemId){
							...
							if(data[i].timeStamp > items[j].timeStamp){
								items[j] = data[i]; 
							}
						}
					}
	...
	__fetchRating: function(itemId){
		$.ajax({
		  method: "GET",
		  url: this.host + this.dataPath + this.sourcePath + this.pathSeparator + this.sourceId + this.usersPath + this.pathSeparator + this.getUserId() + this.itemsPath + this.pathSeparator + itemId,
		  context: {self: this},
		  success: function(data, status, jqXHR) {
