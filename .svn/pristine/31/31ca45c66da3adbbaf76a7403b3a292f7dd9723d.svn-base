...
public class RepoIndex implements ResourceIndexer {
    ...
    private Tag generateResource(File file, Map<String, String> config) throws Exception {
        ...
        Capability bundle = null;
        Capability content = null;
        Capability info = null;

        for (Capability cap : caps) {
            if (cap.getNamespace().equals(Namespaces.NS_WIRING_BUNDLE)) {
                bundle = cap;
            } else if (cap.getNamespace().equals(Namespaces.NS_WIRING_HOST)) {
                host = cap;
            } else if (cap.getNamespace().equals(Namespaces.NS_CONTENT)) {
                content = cap;
            } else if (cap.getNamespace().equals("Bundle-Information")) {
                info = cap;
            }
        }
        ...
        if (info != null && content != null) {
            Map infoAttr = info.getAttributes();
            Map contentAttr = content.getAttributes();


            if (infoAttr.containsKey("Bundle-Name")) {
                resourceTag.addAttribute("presentationname", infoAttr.get("Bundle-Name"));
            }

            if (infoAttr.containsKey("Bundle-SymbolicName") && infoAttr.containsKey("Bundle-Version")) {
                resourceTag.addAttribute("symbolicname", infoAttr.get("Bundle-SymbolicName"));
                resourceTag.addAttribute("version", infoAttr.get("Bundle-Version"));
                resourceTag.addAttribute("id", infoAttr.get("Bundle-SymbolicName") + "/" + infoAttr.get("Bundle-Version"));
            }

            if (contentAttr.containsKey("url")) {
                String baseUrl = config.get(SOURCE_BASE_URL);
                if (baseUrl != null) {
                    String dir = new URI(baseUrl).resolve((String) contentAttr.get("url")).toURL().toString();
                    resourceTag.addAttribute("uri", dir);

                } else {
                    resourceTag.addAttribute("uri", contentAttr.get("url"));
                }
            }

            if (infoAttr.containsKey("Bundle-Description")) {
                Tag elem = new Tag("description");
                elem.addContent((String) infoAttr.get("Bundle-Description"));
                resourceTag.addContent(elem);
            }
            if (infoAttr.containsKey("Bundle-License")) {
                Tag elem = new Tag("license");
                elem.addContent((String) infoAttr.get("Bundle-License"));
                resourceTag.addContent(elem);
            }
            if (infoAttr.containsKey("Bundle-DocURL")) {
                Tag elem = new Tag("documentation");
                elem.addContent((String) infoAttr.get("Bundle-DocURL"));
                resourceTag.addContent(elem);
            }
            if (infoAttr.containsKey("Bundle-Category")) {
                Tag elem = new Tag("category");
                elem.addAttribute("id", (String) infoAttr.get("Bundle-Category"));
                resourceTag.addContent(elem);
            }

            if (contentAttr.containsKey("size")) {
                Tag elem = new Tag("size");
                elem.addContent("" + contentAttr.get("size"));
                resourceTag.addContent(elem);
            }
        ...
        for (Capability cap : caps) {
            if (cap.getNamespace().equals(Namespaces.NS_WIRING_PACKAGE)) {

                Tag capTag = new Tag(Schema.ELEM_CAPABILITY);

                capTag.addAttribute(Schema.ATTR_NAME, "package");

                appendAttributeAndDirectiveTags(capTag, cap.getAttributes(), cap.getDirectives());

                resourceTag.addContent(capTag);
            } else if (cap.getNamespace().equals(Namespaces.NS_SERVICE)) {
                Tag capTag = new Tag(Schema.ELEM_CAPABILITY);

                capTag.addAttribute(Schema.ATTR_NAME, "service");
        ...
        for (Requirement req : reqs) {
            if (req.getNamespace().equals(Namespaces.NS_WIRING_PACKAGE)) {

                Tag reqTag = new Tag(Schema.ELEM_REQUIRE);
                reqTag.addAttribute(Schema.ATTR_NAME, "package");
                String filter = req.getDirectives().get("filter");
                reqTag.addAttribute("filter", filter.replace(Namespaces.NS_WIRING_PACKAGE, "package"));

                String optional = req.getDirectives().get("resolution");
                if(optional != null &&  optional.equals("optional")){
                    reqTag.addAttribute("optional", "true");
            ...
            } else if (req.getNamespace().equals(Namespaces.NS_SERVICE)) {
                Tag reqTag = new Tag(Schema.ELEM_REQUIRE);
                reqTag.addAttribute(Schema.ATTR_NAME, "service");
                String filter = req.getDirectives().get("filter");

                reqTag.addAttribute("filter", filter.replace("objectClass", "service"));
    ...
    private static void appendAttributeAndDirectiveTags(Tag parentTag, Map<String, Object> attribs, Map<String, String> directives) {
        for (Entry<String, Object> attribEntry : attribs.entrySet()) {
            Tag attribTag = new Tag(Schema.ELEM_P);
            if(attribEntry.getKey().equals(Namespaces.NS_WIRING_PACKAGE)){
                attribTag.addAttribute(Schema.ATTR_N, "package");
            } else {
                attribTag.addAttribute(Schema.ATTR_N, attribEntry.getKey());
            }
