...
@Configuration(key = "beta", type = ConfigurationItem.ConfigurationItemType.Double,  description = "time decay factor")
@Configuration(key = "numBins", type = ConfigurationItem.ConfigurationItemType.Integer,  description = "number of bins over all the items")
public class TimeSVD extends IterativeRecommender {
    ...
    public double predict(int u, int i) throws Exception {
		...
		long timestamp = (long) timeMatrix.get(u, i); //TODO test this originally this was the testTimeMatrix
		int t = days(timestamp, minTimestamp);
		int bin = bin(t);
		double dev_ut = dev(u, t);
