...
/**
 * This class is used for comparison in experiments and provides a user average as prediction.
 * The recommend method provides a empty list.
 *
 * @author Matthäus Schmedding (info@recalot.com)
 */
public class GlobalUserAverageRatingRecommender extends Recommender {
    ...
    public void train() throws BaseException {
        ...
        for (Interaction interaction : getDataSet().getInteractions()) {
            if(interaction.getType().toLowerCase().equals(ratingType)) {
                ...
                if(value != null) {
                    Helper.incrementMapValue(count, interaction.getUserId());
                    Helper.incrementMapValue(sum, interaction.getUserId(), 1.0 * value);
                }
            }
        }
        ...
        for(String userId : count.keySet()) {
            int c = count.get(userId);
            double s = sum.get(userId);

            if(c > 0) {
                prediction.put(userId, s / c);
            }
        }
    }
